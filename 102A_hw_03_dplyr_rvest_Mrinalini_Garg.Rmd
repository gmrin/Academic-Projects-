...
```{r exc1, error = TRUE}
ttt<- function(x= as.character(1:9)){
  if(  !is.character(x) | !is.atomic(x) ) {
   return(structure(as.character(1:9), class="ttt") ) 
    }
  return(structure(x, class="ttt"))
}

#Defining generic function
print<- function(x)  UseMethod("print")

#Defining method for class ttt
print.ttt<- function(place){
...
case1 <- rep(as.character(1:9), 9)  # or if you must, as.character(1:9)
case1[c(1,3)] <- "x"
case1[2] <- "o"
case1 <- ttt(case1)
...
?
```{r exc2c}
vehicles%>% filter(year == 2014) %>% group_by(class)%>% summarize(Avg_cty= mean(cty))-> c

c[ c$class== "Compact Cars" | c$class== "Midsize Cars", ]
...
vehicles[ vehicles$class== "Midsize Cars" & vehicles$year== 2014,] %>% group_by(make)%>% summarize(Avg_cty= mean(cty)) %>% arrange(desc(Avg_cty))
...
vehicles[ vehicles$class== "Midsize Cars",] %>% filter(year == 1994 | year == 1999 |year == 2004 |year == 2009 |year == 2014) %>% group_by(year, make)%>% summarize( Avg_cty= mean(cty)) %>% arrange(desc(Avg_cty)) -> e

# Creating final table: 
spread( e, year, Avg_cty)
...
```{r exc3}
load("dr4.Rdata") 
dr4%>% filter(!is.na(visit2)) %>% 
  transmute( first= visit2- visit1, second= visit3-visit2 , third= visit4-visit3, fourth= visit5- visit4, count2= !is.na(visit2), count3= !is.na(visit3), count4= !is.na(visit4), count5= !is.na(visit5), total_vis= count2+ count3+ count4 + count5) -> rep_vis
rep_vis[is.na(rep_vis)]<- 0
rep_vis<- rep_vis %>% mutate(total= first+second+third+ fourth)

# Average time between visits
sum(rep_vis$total)/ sum(rep_vis$total_vis)
...
# List of teams:
teams<- teampage %>%  html_nodes("#teams_active .left a") %>% html_text()

# Creating pathway 
s <- html_session("http://www.baseball-reference.com/teams/") # Starts a session 
baseball<- data.frame()

# loop to get all team franchise history tables 
for(i in teams[1:30]){
  teampage <- s %>% follow_link(i) %>% read_html()
    
  tabtemp<-  teampage %>% html_node("#franchise_years")%>% 
      html_table() 
    
  tabtemp<- cbind(  tabtemp,  rep(i, length(dim(tabtemp)[2])))
   
   baseball<- rbind( baseball, tabtemp)
}

colnames(baseball)[22]<- "current"
...
# Creating table
baseball %>% filter(Year %in% c(2001:2016)) %>% group_by(current) %>%  summarise(totalwins= sum(W), totallosses= sum(L), total_runs= sum(R), total_RA= sum(RA), wins_perc= round(sum(W)/(sum(L)+ sum(W)),4)) %>% arrange(desc(wins_perc))-> tab_sum

print.data.frame(tab_sum)
...
```{r exc6}
 library(stringr)
# Giving each manger for each season a separate column 
#baseball2<- baseball %>%  filter(Year %in% c(2001:2016))
main<- baseball$Managers %>% 
  str_match_all(. , "[[:alpha:]]\\.[[:alpha:]]+\\s*([[:alpha:]]+)*\\s\\(\\d+-\\d+\\)") %>%  unlist

name<-as.character(str_extract(main,"[[:alpha:]]\\.[[:alpha:]]+\\s*([[:alpha:]]+)*" ))

wins<- main %>% str_extract("\\s\\(\\d+") 
wins<- as.numeric(str_extract(wins, "\\d+"))

losses<- main %>%  str_extract("-\\d+\\)")
losses<-as.numeric(str_extract(losses, "\\d+"))

manager<- as.data.frame(cbind(name, wins, losses))
manager$wins<- as.numeric(as.character(manager$wins))
manager$losses<- as.numeric(as.character(manager$losses))

manager %>% group_by(name) %>% summarise(Total= sum(wins)+ sum(losses) , W= sum(wins), L= sum( losses), Win_perc= W/Total) %>% arrange(desc(Total)) %>% head(10)
